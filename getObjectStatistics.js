...
function getObjectStatistics(object, key='__roo_t_', first=true) {
    ...
    let min, firstObjKey, secondObjKey
    if (Array.isArray(object) && !isNaN(min = Math.min.apply(null, object))) {
        ...
        return ret
    } else if (Array.isArray(object) && object.length > 1 && (firstObjKey = Object.keys(object[0]))[0] === (secondObjKey = Object.keys(object[1]))[0]) {
        let firstObjectKeys = Object.keys(object[0])
        let secondObjectKeys = Object.keys(object[1])
        const intersection = firstObjectKeys.filter(value=>secondObjectKeys.includes(value));
        const ret = {}
